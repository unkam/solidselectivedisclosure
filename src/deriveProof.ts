...
import {
  ...
  import keyPairOptions from "../holder/keyPair.json";
...
import { callbackify } from "util";
  ...
  const main = async (): Promise<void> => {
    ...
    fs.writeFile(path.join(__dirname, "./data/derivedProof.json"), JSON.stringify(derivedProof, null, 2), err => {
      if (err) {
        console.error(err)
        return
        }
